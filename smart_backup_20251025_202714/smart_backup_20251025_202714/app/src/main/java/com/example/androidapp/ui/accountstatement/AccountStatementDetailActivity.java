...
public class AccountStatementDetailActivity extends AppCompatActivity {
    ...
    private void loadAccountStatementDetails(int id) {
        viewModel.getAccountStatementById(id, companyId, accountId).observe(this, statement -> {
            if (statement != null) {
                etTransactionDate.setText(statement.getDate());
    ...
    private void saveAccountStatement() {
        ...
        float debit = Float.parseFloat(etDebit.getText().toString().isEmpty() ? "0" : etDebit.getText().toString().trim());
        float credit = Float.parseFloat(etCredit.getText().toString().isEmpty() ? "0" : etCredit.getText().toString().trim());
        ...
        AccountStatement accountStatement = new AccountStatement(companyId, accountId, transactionDate, description, debit, credit, 0.0f, referenceType, referenceId);
        if (statementId == -1) {
            viewModel.insert(accountStatement);
