...
public interface RolePermissionDao extends BaseDao<RolePermission> {
    ...
    @Query("SELECT * FROM role_permission WHERE role_id = :roleId")
    List<RolePermission> getByRoleId(String roleId);

    @Query("SELECT * FROM role_permission WHERE permission_id = :permissionId")
    List<RolePermission> getByPermissionId(String permissionId);

    @Query("SELECT COUNT(*) FROM role_permission WHERE role_id = :roleId")
    int getPermissionCountByRole(String roleId);

    @Query("DELETE FROM role_permission WHERE role_id = :roleId")
    void deleteByRoleId(String roleId);

    @Query("DELETE FROM role_permission WHERE permission_id = :permissionId")
